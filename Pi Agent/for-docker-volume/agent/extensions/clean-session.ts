 ...
 *
 * Adds a /clean command that deletes the current session and starts a
 ...
 *
 * Usage:
 *   /clean   — replace the current session with a fresh one
 */
...
/**
 * Delete a session file: try `trash` CLI first (recycle bin),
 * then fall back to unlink.
 */
